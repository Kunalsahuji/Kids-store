...
export function HeroSection() {
  ...
  const mobileHeroImage = 'https://images.pexels.com/photos/6148507/pexels-photo-6148507.jpeg?auto=compress&cs=tinysrgb&w=1200&h=900&fit=crop'

  return (
    <section id="home" className="section-pad pb-1 pt-3 md:pt-4">
      <div className="container-shell">
        <article className="overflow-hidden rounded-[14px] bg-[#d7e3f4] px-5 pt-6 shadow-[inset_0_0_0_1px_rgba(34,34,34,0.06)] md:hidden">
          <h1 className="whitespace-pre-line text-center font-[var(--font-display)] text-[34px] leading-[0.98] text-[#2f2a3b]">{heroTitle}</h1>
          <div className="mt-4 h-[178px] overflow-hidden rounded-t-[999px] rounded-b-[12px] bg-[#c7dceb]">
            <img className="h-full w-full object-cover object-[50%_42%]" src={mobileHeroImage} alt={heroBanner.title} loading="eager" />
          </div>
        </article>
        ...
        <div className="mx-auto hidden max-w-[1328px] gap-5 md:grid md:grid-cols-[1fr_400px] md:gap-[30px]">
          <article className="overflow-hidden rounded-[14px] bg-[#d7e3f4] shadow-[inset_0_0_0_1px_rgba(34,34,34,0.06)] md:h-[272px] md:grid md:grid-cols-[1.55fr_1fr]">
            <div className="flex h-full flex-col items-center justify-center px-6 py-7 md:px-12 md:py-0">
              ...
              <button className="mt-6 hidden rounded-full bg-[#2f2a35] px-7 py-3 text-[15px] font-semibold text-white md:inline-flex" type="button">
                {heroBanner.cta}
              </button>
            </div>
            <div>
              <img className="h-full w-full object-cover" src={heroBanner.imageUrl} alt={heroBanner.title} loading="eager" />
            </div>
          </article>

          <article className="relative hidden h-[272px] overflow-hidden rounded-[14px] md:block">
            <img className="h-full w-full object-cover" src={heroSecondaryCard.imageUrl} alt={heroSecondaryCard.title} loading="lazy" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/65 via-black/22 to-black/6" />
